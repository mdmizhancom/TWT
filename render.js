...
const Render = {
  updateAdmin() {
    ...
    if (p) p.className = AppState.isAdmin ? "badge-pill admin" : "badge-pill viewer";
    if (p) p.innerHTML = AppState.isAdmin ? '<i class="fa-solid fa-lock-open"></i> অ্যাডমিন মোড' : '<i class="fa-solid fa-eye"></i> ভিউয়ার মোড';
  ...
  dropdowns() {
    const sOpts = '<option value="all">সব বিল্ডিং / সাইট</option>' + AppState.sites.map(s => `<option value="${s.id}" data-name="${s.name}">${s.name}</option>`).join('');
    const wOpts = '<option value="all">সকল কারিগর / শ্রমিক</option>' + AppState.workers.map(w => `<option value="${w.id}" data-name="${w.name}" data-rate="${w.daily_rate}" data-role="${w.role}">${w.name} (${w.role})</option>`).join('');
    document.querySelectorAll(".select-site-options").forEach(el => el.innerHTML = el.classList.contains("filter-select") ? sOpts : sOpts.replace('value="all"', 'value="" disabled selected'));
    document.querySelectorAll(".select-worker-options").forEach(el => el.innerHTML = el.classList.contains("filter-select") ? wOpts : wOpts.replace('value="all"', 'value="" disabled selected'));
  },
  ...
  dashboard() {
    ...
    document.getElementById("dashboard-recent-logs").innerHTML = rec.length ? `
      <div class="table-responsive"><table class="custom-table"><thead><tr><th>তারিখ</th><th>শ্রমিক</th><th>বিল্ডিং</th><th>মজুরি</th><th>অগ্রিম</th><th>কাজ</th></tr></thead>
      <tbody>${rec.map(l => { const w = AppState.workers.find(i => i.id === l.worker_id); return `<tr><td><strong>${l.date}</strong></td><td><div class="worker-row-info">${AppState.getAvatar(w, 'worker-avatar-sm')} <strong>${l.worker_name}</strong></div></td><td><span class="badge-pill viewer">${l.site_name}</span></td><td class="money-green">${AppState.money(l.wage_amount)}</td><td class="money-amber">${+l.advance_paid ? AppState.money(l.advance_paid) : '-'}</td><td>${l.work_sqft ? l.work_sqft + ' sqft' : (l.remarks || '-')}</td></tr>`; }).join('')}</tbody></table></div>`
      : '<div class="empty-state"><p>কোনো রেকর্ড পাওয়া যায়নি।</p></div>';
  },
  ...
  logs() {
    ...
    el.innerHTML = `<div class="table-responsive"><table class="custom-table"><thead><tr><th>তারিখ</th><th>বিল্ডিং</th><th>শ্রমিক</th><th>রোল</th><th>মজুরি</th><th>অগ্রিম</th><th>বকেয়া</th><th>কাজ</th><th>নোট</th><th class="admin-only" style="text-align:right">অ্যাকশন</th></tr></thead><tbody>
      ${list.map(l => { const w = AppState.workers.find(i => i.id === l.worker_id); return `<tr><td><strong>${l.date}</strong></td><td><span class="badge-pill viewer">${l.site_name}</span></td><td><div class="worker-row-info">${AppState.getAvatar(w, 'worker-avatar-sm')} <strong>${l.worker_name}</strong></div></td><td><span class="tag-badge tag-mistri">${l.role}</span></td><td class="money-green">${AppState.money(l.wage_amount)}</td><td class="money-amber">${+l.advance_paid ? AppState.money(l.advance_paid) : '-'}</td><td class="${(+l.wage_amount - +l.advance_paid) > 0 ? 'money-rose' : 'money-green'}">${AppState.money(+l.wage_amount - +l.advance_paid)}</td><td>${l.work_sqft ? l.work_sqft + ' sqft' : '-'}</td><td style="color:#64748b;font-size:0.8rem">${l.remarks || '-'}</td><td class="admin-only" style="text-align:right"><button class="btn btn-secondary btn-icon btn-sm edit-log-btn" data-id="${l.id}"><i class="fa-solid fa-pen"></i></button> <button class="btn btn-outline-danger btn-icon btn-sm delete-log-btn" data-id="${l.id}"><i class="fa-solid fa-trash"></i></button></td></tr>`; }).join('')}
    </tbody></table></div>`;
  },
  ...
  workerProfile(w) {
    ...
    document.getElementById("prof-role").textContent = w.role || "কারিগর";
    document.getElementById("prof-phone").textContent = w.phone || "মোবাইল নম্বর নেই";
    document.getElementById("prof-location").textContent = w.location || "ঠিকানা দেওয়া হয়নি";
    document.getElementById("prof-rate").textContent = AppState.money(w.daily_rate) + " / দিন";
  ...
  expenses() {
    ...
    el.innerHTML = `<div class="table-responsive"><table class="custom-table"><thead><tr><th>তারিখ</th><th>বিল্ডিং</th><th>খাত</th><th>পরিমাণ</th><th>বিবরণ</th><th class="admin-only" style="text-align:right">অ্যাকশন</th></tr></thead><tbody>
      ${list.map(e => `<tr><td><strong>${e.date}</strong></td><td><span class="badge-pill viewer">${e.site_name}</span></td><td><span class="tag-badge tag-cutter">${e.category}</span></td><td class="money-rose">${AppState.money(e.amount)}</td><td style="color:#64748b">${e.note || '-'}</td><td class="admin-only" style="text-align:right"><button class="btn btn-secondary btn-icon btn-sm edit-expense-btn" data-id="${e.id}"><i class="fa-solid fa-pen"></i></button> <button class="btn btn-outline-danger btn-icon btn-sm delete-expense-btn" data-id="${e.id}"><i class="fa-solid fa-trash"></i></button></td></tr>`).join('')}
    </tbody></table></div>`;
  },
